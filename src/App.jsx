...
const API_KEY = 'cae463b002e67c07ee7a1de6bbd1f032'
  ...
  return (
    ...
    <div className='min-h-screen'>
     ...
     <div className='bg-transparent backdrop-filter backdrop-blur-md rounded-xl
      ...
      {!weather ? (<form onSubmit={handleSearch} className='flex flex-col relative '>
        ...
        {suggestion.length >0 &&(
          <div className='absolute top-12 left-0 right-0 bg-transparent shadow-md rounded z-10'>
            {suggestion.map((s)=>(
              <button type='button' key={`${s.lat}-${s.long}`}
        ...
        <button type='submit' className='bg-purple-700 hover:bg-blue-700 text-white font-semibold
        py-2 px-4 rounded transition-colors'>Get Weathers</button>
      </form>):(
        <div className='mt-6 text-center transition-opacity duration-500'>
          <button onClick={()=>{setWeather(null); setCity('')}} className='bg-purple-900 hover:bg-blue-700 text-white font-semibold
        ...
        py-1 px-3 rounded transition-colors'>&deg;{unit}</button>
          </div>
          ...
          <div className='flex flex-wrap justify-around mt-6'>
           {[
            ...
            <p className='mt-1 font-semibold'>{label}</p>
            <p className='text-sm'>
  {new Date(time * 1000).toLocaleString('en-GB', { hour: '2-digit', minute: '2-digit' })}
</p>
          ...
          <div className='mt-6 text-sm'>
            <p><strong>Feels Like:</strong>{convertTemperature(weather.main.feels_like, unit)}</p>
